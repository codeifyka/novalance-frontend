...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (response.isSuccess() && response.value) {
        pendingProposals.value =
          response.value.filter((p) => p.status === "pending") || [];
        doneProposals.value =
          response.value.filter((p) => p.status === "done") || [];
        activeProposals.value =
          response.value.filter((p) => p.status === "active") || [];
      }
    ...
    const handleProposalAccepted = (proposalId: number) => {
      doneProposals.value.push(
        ...pendingProposals.value.filter(
