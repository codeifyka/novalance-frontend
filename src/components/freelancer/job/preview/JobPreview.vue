...
try {
    const getByProposalByJobIdResult = await restProposals.getByProposalByJobId(Number(route.params.id));
    if (getByProposalByJobIdResult.isSuccess()) {
        console.log("result:", getByProposalByJobIdResult.value);
        isSent.value = true
    }
}
catch (error) {
